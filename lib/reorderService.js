...
import { buildStore } from "./apiVersion.js";
import {
  upsertConfig,
  listStoresInternal,
  listEnabledConfigs,
...
export async function runReorder({
  ...
  return withLock(store.id, collectionGid, async () => {
    ...
    const { finalOrder, newGroups } = sortCollection(products, productTypeOrder, stockThreshold, manualProductOrder);
    ...
    for (const config of configs) {
      ...
      try {
        const result = await runReorder({
          ...
          manualProductOrder: config.manual_product_order || {},
          save: false,
        });
