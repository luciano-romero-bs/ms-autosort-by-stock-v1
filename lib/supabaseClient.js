...
export async function upsertConfig({
  ...
  // Only touch new_product_types/manual_product_order when the caller says
  // so — an upsert that omits them leaves the existing values intact (e.g.
  // the "Automatizaciones guardadas" editor only ever touches order+umbral).
  if (newProductTypes !== undefined) row.new_product_types = newProductTypes;
  if (manualProductOrder !== undefined) row.manual_product_order = manualProductOrder;
